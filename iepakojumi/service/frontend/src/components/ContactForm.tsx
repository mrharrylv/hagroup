...
const LAST_SUBMIT_KEY = 'iepakojumi-contact-last-submit';
...
export default function ContactForm({ selectedProduct, onProductChange }: ContactFormProps) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      await addDoc(collection(db, 'iepakojumi_contacts'), {
      ...
      console.error('[Iepakojumi contact] Could not submit enquiry:', error);
      setStatus('error');
    }
  };
