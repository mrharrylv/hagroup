...
resource "aws_cloudfront_distribution" "website" {
  ...
  # Dedicated origin for permanent brand assets (logo, favicon). Uploaded
  # manually and never touched by the website deploy pipeline — see assets.tf.
  origin {
    domain_name              = aws_s3_bucket.assets.bucket_regional_domain_name
    origin_id                = "S3-${local.assets_bucket_name}"
