...
const STORAGE_KEY = 'iepakojumi-cookie-notice';
...
export default function CookieBanner() {
  ...
  useEffect(() => {
    window.addEventListener('iepakojumi-reopen-cookies', reopen);
    return () => window.removeEventListener('iepakojumi-reopen-cookies', reopen);
  }, [reopen]);
  ...
  const acknowledge = async () => {
    ...
    try {
      ...
      if (db) {
        const record = await addDoc(collection(db, 'iepakojumi_consents'), {
      ...
      console.error('[Iepakojumi cookies] Could not store acknowledgement:', error);
      setStatus('error');
    }
  };
