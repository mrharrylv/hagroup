...
/** The switcher's link for a language: the desktop dropdown's, or the mobile menu's once opened. */
async function languageLink(code: Lang, menu: Menu): Promise<HTMLAnchorElement> {
  ...
  const links = languageLinks().filter((link) => link.hreflang === code);
  const link = menu === 'desktop' ? links[0] : links[links.length - 1];
